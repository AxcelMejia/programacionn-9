...
if (res)
    fputs("\n", ar);
/* Se indica un salto de línea, excepto en la última cadena. Si no se hiciera esta indicación, la función fputs pegaría las cadenas y luego tendríamos dificultades en el momento de leerlas.
Por otra parte, si realizáramos este salto de línea al final de la última cadena, en la escritura se repetiría la última cadena. */
}
   fclose(ar);
}
   else
