...
/* Esta función cuenta el número de minúsculas y mayúsculas que hay en el archivo aarrcc.. */
{
    int min = 0, may = 0;
    char p;
    while (!feof(arc))
{
    p = fgetc(arc); /* Se utiliza la función ffggeettcc(()) para leer caracteres del archivo. */
    if (islower(p))
    min++;
    else
    if (isupper(p))
